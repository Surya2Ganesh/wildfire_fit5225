"""Main Streamlit page for interactive wildfire detection demos."""
...
# Prevent an OpenMP duplicate library warning that can happen on Windows.
os.environ['KMP_DUPLICATE_LIB_OK'] = 'True'

# Load a model once per Streamlit session instead of reloading on every rerun.
@st.cache_resource
def load_model(model_path):
    """Create and cache a YOLO model for the selected weights file."""
    model = YOLO(model_path)
    return model

# Run inference and prepare both the annotated image and summary text.
def predict_image(model, image, conf_threshold, iou_threshold):
    # Ask YOLO to detect objects using the user-selected thresholds.
    res = model.predict(
        image,
        conf=conf_threshold,
        iou=iou_threshold,
    ...
    # Count how many times each detected class appears in the current image.
    for c in classes:
        c = int(c)
        class_counts[class_name[c]] = class_counts.get(class_name[c], 0) + 1

    # Build a short human-readable detection summary for the UI.
    ...
    # Ultralytics reports stage timing in milliseconds, so convert to seconds.
    latency = sum(res[0].speed.values())  # in ms, need to convert to seconds
    latency = round(latency / 1000, 2)
    prediction_text += f' in {latency} seconds.'

    # result.plot() returns a BGR OpenCV image, while Streamlit expects RGB.
...
def main():
    # Configure the browser tab and sidebar behavior before drawing the page.
    st.set_page_config(
    ...
    with col1:
        st.write("")
    with col2:
        # Pick a random generated logo each time the app starts.
    ...
    # Switch between wildfire-specific weights and generic COCO weights.
    ...
    # Build the chosen weight path and load or reuse the cached model.
    ...
    if image_source == "Upload from Computer":
        # Let the user choose a local image file from their machine.
        ...
        # Download the image from a URL when the user pastes one.
    ...
    if image:
        # Run inference only after a valid image has been supplied.
        ...
        # Keep the rendered prediction in memory so the user can download it.
        image_buffer = io.BytesIO()

        # PNG preserves the annotations without writing a temporary file to disk.
        prediction.save(image_buffer, format='PNG')

        # Expose the generated image through Streamlit's built-in download control.
        st.download_button(
